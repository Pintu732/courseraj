    ...
    <article class="course-card">
      <a class="course-media" href="course.html?id=${encodeURIComponent(c.id)}">
        ${image ? `<img src="${esc(image)}" alt="${esc(title)}" loading="lazy">`
                : `<div class="placeholder-art">${esc(c.category||"COURSE")}</div>`}
      ...
      <div class="course-meta">
        <span>${esc(c.category || "Course")}</span>
        <strong>${money(c.price) || "Ask Price"}</strong>
      </div>
...
function renderTrending(){
  ...
  $("#trendingRow").innerHTML=list.length
    ...
    : `<div class="loading-state">Add courses from Admin → Courses. Mark them Trending to show them here.</div>`;
}

function renderCollage(){
  const live=courses.filter(c=>c.published!==false);
  const featured=live.filter(c=>c.featured);
  const list=(featured.length?featured:live).slice(0,8);
  if(!list.length) return;

  const classes=["card-a","card-b","card-c","card-d","card-e","card-f","card-g","card-h"];
  $("#heroCollage").innerHTML=list.map((c,i)=>{
    const img=imageOf(c);
    return `<article class="float-card ${classes[i]||""}">
      ${img ? `<img src="${esc(img)}" alt="${esc(c.title||"Course")}">`
            : `<div class="placeholder-art">${esc(c.category||"SKILL")}</div>`}
    </article>`;
  }).join("");
}
...
function renderCategories(){
  ...
  document.querySelectorAll("[data-cat]").forEach(btn=>{
    btn.onclick=()=>{
      $("#heroSearch").value=btn.dataset.cat;
      renderSearch(btn.dataset.cat);
      $("#heroSearch").focus();
    }
  });
}
