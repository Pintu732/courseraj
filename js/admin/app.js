...
function editCourse(id){
  ...
  ["title","creator","category","price","oldPrice","rating","badge","image","telegram","description","order"].forEach(k=>$("#"+k).value=c[k]??"");
...
$("#courseForm").onsubmit=async e=>{
  e.preventDefault(); const msg=$("#courseSaveMsg"),btn=$("#saveCourseBtn");btn.disabled=true;msg.textContent="";
  const payload={
    title:$("#title").value.trim(),creator:$("#creator").value.trim(),category:$("#category").value.trim(),
...
function renderCategories(){
  ...
  <article class="mini-row"><div><b>${esc(c.name||"Unnamed")}</b><small>Order ${c.order??9999} • ${c.visible===false?"Hidden":"Visible"}</small></div>
  <div><button data-edit-cat="${c.id}">Edit</button><button class="danger" data-delete-cat="${c.id}">Delete</button></div></article>`).join(""):`<div class="empty">No categories yet.</div>`;
...
function resetCat(){ $("#categoryForm").reset();$("#categoryId").value="";$("#categoryVisible").checked=true;$("#categoryFormTitle").textContent="Add Category"; }
...
function editCategory(id){
  ...
  $("#categoryEditor").classList.remove("hidden");$("#categoryFormTitle").textContent="Edit Category";$("#categoryId").value=id;$("#categoryName").value=c.name||"";$("#categoryOrder").value=c.order??9999;$("#categoryVisible").checked=c.visible!==false;
}
async function deleteCategory(id){if(confirm("Delete this category? Courses will not be deleted."))await removeCategory(id);}
$("#categoryForm").onsubmit=async e=>{
  e.preventDefault();await saveCategory($("#categoryId").value,{name:$("#categoryName").value.trim(),order:Number($("#categoryOrder").value)||9999,visible:$("#categoryVisible").checked});$("#categoryEditor").classList.add("hidden");
};
...
watchCourses(list=>{courses=list;refreshStats();renderCourseList();},console.error);
watchCategories(list=>{categories=list;refreshStats();renderCategories();},console.error);
